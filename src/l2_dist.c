/*  The program "l2_dist" compares two phylogenetic trees and calculates 
    L2-distance (Penny's Path Difference Metric), i.e. the mean square difference between path lengths
    for all pairs of leafs. Here path length is the number of branches on
    the path from one leaf to another, so this metric is purely topological and does not depend
    on branch lengths.
    ...
    For compilation, l2_dist requires this file, treedist.c and treedist.h.
...
int main (int argc, char *argv[])
{
  ...
  if (strcmp(argv[1], "-h") == 0 ||
      ...
      strcmp(argv[1], "--help") == 0 ) {
    fprintf(stderr, "l2_dist is a program computing L2 distance ");
  ...
  if ( n == intree2.leavesnum ) 
    distance = treedist2(intree1, intree2, 2);
  if ( n < intree2.leavesnum ) 
    distance = treedist2(intree1, subtree(intree2, intree1.leaf, n), 2);
  if ( n > intree2.leavesnum ) {
    n = intree2.leavesnum; 
    distance = treedist2(intree2, subtree(intree1, intree2.leaf, n), 2);
  }
  if (distance >= 0 ) 
    printf("%.4f\n", sqrt( (float)(distance * 2)/(n * (n - 1)) ) );
  else puts("1000000");
  return 0;
} /* main */
